...
| `package-adm-wav` fails before or during stem writing | Check free disk space first. Large Atmos package generation writes one mono float32 stem per node and may require several GiB of free space. If the source is `RF64`/`BW64` and you see `Short read while splitting source WAV`, verify you're on the May 11, 2026 `ds64` fix in `packagingHelper.hpp`. |
